...
export function demoBrandSeeded(): boolean {
  const dir = dataDir();
  const brands = readJson(path.join(dir, "brands.json"));
  return brands.some((b) => b.slug === DEMO_SLUG);
}

export function seedDemoBrand(): { ok: boolean; counts?: Record<string, number>; images?: number; error?: string } {
